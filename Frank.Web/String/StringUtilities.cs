...
namespace Frank.Web.String
{
    public static class StringUtilities
    {
        ...
        {
            var result = text.Replace(removeText, string.Empty);
            return result;
        }

        public static string ToShortString(this string text, int maxSize = 50)
        {
        ...
        public static string JoinString(this IEnumerable<string> source, string split = ", ")
        {
            var result = source.Aggregate((i, j) => i + split + j);
            return result;
        }

        public static string ToEmptyIfNull(this object value)
        {
            if (value == null)
            {
                return "";
        ...
        public static string VietnameseString(this string str)
        {
        ...
        public static string GetUserName2(this string obj)
        {
            string result = string.Empty;
        ...
        {
            string[] arrName = { "Chủ nhật", "Thứ 2", "Thứ 3", "Thứ 4", "Thứ năm", "Thứ 6", "Thứ 7" };

            return string.Format("{0}, {1}", arrName[(int)dateTime.DayOfWeek], dateTime.ToString("dd/MM"));

        }

        public static string DayOfWeekVNLong(this DateTime dateTime)
        {
            string[] arrName = { "Chủ nhật", "Thứ 2", "Thứ 3", "Thứ 4", "Thứ năm", "Thứ 6", "Thứ 7" };

            return string.Format("{0}", arrName[(int)dateTime.DayOfWeek]);

        }


        public static string DayOfWeekVN(this DateTime dateTime)
        {
            string[] arrName = { "CN", "T2", "T3", "T4", "T5", "T6", "T7" };

            return string.Format("{0}", arrName[(int)dateTime.DayOfWeek]);

        }

        public static string JoinCharter(this string[] obj, char character)
        {
        ...
        public static List<string> ConvertToList(this string data, char character)
        {
            if (string.IsNullOrEmpty(data)) return new List<string>();
            var listSplit = data.Split(character);
        ...
        public static bool CheckInStringHaveComma(this string StringHaveComma, List<string> ListCheck)
        {
            if (!string.IsNullOrEmpty(StringHaveComma) && ListCheck.Any())
            {
                var lstCheckOld = StringHaveComma.Split(',');

                return lstCheckOld.Any(x => ListCheck.Contains(x));
            }
            else
            {
                return false;
        ...
        public static string ConvertToRoman(this int number)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("Number out of range for Roman numerals (1-3999).");
            }
